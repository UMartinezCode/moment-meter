...
`totals()` over the current period's bounds (`goalBounds(period)`). `renderGoals()` shows a
progress bar (guarded against a 0 target → no divide-by-zero); a met goal shows an inline green
check that lives in the row's normal flow, before the period pill, so it never overlaps the
label. `openGoalSheet()` picks the category with the shared type-to-search combobox (§13,
prefix `"g"`). `checkGoalCelebrate()` toasts once when a goal is first reached (tracked in an
in-memory `celebrated` set keyed by goal id + period start).
