...
| File | What it is |
|------|-----------|
| `index.html` | **The entire app** — HTML + CSS + JS in one file (~2,500 lines). |
...
| `Pre-existing Data/`, `Categories/`, `Existing Reference/` | Source material used to seed the app (real CSV exports + screenshots). Not shipped. |
...
`makeSeed()` builds the initial dataset from the user's real CSV exports (hard-coded in
`SEED_CATS` / `SEED_DAYS`). `load()` reads storage, runs migrations, and falls back to the
seed on first run or corruption. `save()` writes storage and (if sync is on) schedules a
debounced push.
...
status/actions, data export/import, archived-category restore, Shortcuts help, and the
danger zone, ending in a footer with the `APP_BUILD` tag. (The old "Lock Screen live timer"
toggle was removed — see §7.)
...
`S.goals` = `{catId, period:"day"|"week"|"month", target:<minutes>}`. `goalProgress(g)` uses
`totals()` over the current period's bounds. `renderGoals()` shows a progress bar (guarded
against a 0 target → no divide-by-zero). `checkGoalCelebrate()` toasts once when a goal is
first reached (tracked in an in-memory `celebrated` set keyed by goal id + period start).
...
6. **Merge remote goals** (dedupe by `catId|period`).
...
- `exportAll()` — one CSV with events + category totals + goals (opens the share sheet).
...
- **State/seed:** `makeSeed`, `load`, `save`, `isPristineOldInstall`, `ensureFamilies`,
  `layoutDay`, `hms`.
...
- **Goals:** `goalBounds`, `goalProgress`, `checkGoalCelebrate`, `openGoalSheet`.
- **Sheets/dialogs:** `openSheet`, `closeSheet`, `sheetOpen`, `askConfirm`, `openCatSheet`,
  `openEventSheet`, `openTimerSheet`, `openGoalSheet`, `openShortcutSheet`, `deleteEvent`.
...
- **Import/export:** `exportAll`, `exportJSON`, `importCSV`, `importJSON`, `parseCSV`,
  `download`, `markBackedUp`, `lastBackupLabel`.
