...
shared type-to-search combobox (§13, prefix `"g"`) whose option list is **families (tagged)
plus categories** — family option ids are encoded `"fam:<name>"`, category options are the raw
`catId`, and the save handler decodes which was chosen. `checkGoalCelebrate()` toasts once when
