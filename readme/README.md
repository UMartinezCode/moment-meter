...
**Goals** — daily / weekly / monthly time goals, set on either a single **category** or a whole **family** (a family goal sums all its categories), with progress bars; a green check marks a goal once it's met. The picker is the same type-to-search box as the Log — start typing to narrow it, with families listed and tagged alongside categories.

**Plus:** editable categories and families, a printable **PDF report** and a comprehensive **CSV export**, JSON backup/restore, CSV import (Timelines format), automatic dark mode, and Siri Shortcuts control. Timers survive closing the app, and everything works offline.

## Reports & export

Two ways to get your data out, both under Settings → Data:

**PDF report** builds a clean, printable summary — a cover with your date range, four headline stats (total tracked, days tracked, daily average, active categories), a family-mix donut, per-family and per-category breakdowns with share bars, a day-by-day stacked timeline, and goal progress. It opens as a preview; tap **Save as PDF** to use your browser's print-to-PDF (on iPhone: Save as PDF → Share → Save to Files; on Mac: the print dialog). No internet or extra apps needed.

**Export CSV** writes one file covering everything: a summary block, every individual event, per-category totals with family and share, per-family totals, a full day-by-day breakdown across your whole history, and goals with current progress. **Backup (JSON)** is a full restorable snapshot (the sync token is never included).
...
A brand-new install (a device that hasn't connected sync yet) starts with the default category and family tiles above but **no history** — the Log and Stats are empty until you start tracking. There's no bundled sample data; every device shows the same thing. Connect sync (below) and your real data merges in from your private gist.

**Importing past exports:** Settings → Import CSV understands the Timelines daily-export format (date + category columns). It matches your existing categories, creates any new ones with sensible colors, and skips days already imported — so you can re-import overlapping files safely. This is how you bring in historical data.
...
- The Settings export row shows how long ago you last exported (CSV or JSON), and the app nudges you every two weeks — browser storage can be evicted by iOS if a site sits unused, so occasional backups are your safety net. See **Reports & export** above for what each option produces.
